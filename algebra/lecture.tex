...
\begin{document}
    ...
    Если $X'  =E_i, Y' = E_j$, то $(X')^TY' = d_{ij},\ 
    ...
    \begin{proof}
        $$\begin{cases}
            ...
            f(x,y) = f_+(x,y) - f_-(x,y)
        \end{cases}$$
        $$f_+(x,y) = \frac{f(x, \overline{y}) + f(y,x)}{2},\ 
    ...
    \begin{definition}
        ...
        \not\equiv 0$.
    \end{definition}
    Обратим внимание, что если $f(x,y) = f_+(x,y)$, то 
    ...
    \end{proof}
    Координатная запись: $$k(x) = X^TB_eX = \sum\limits_{i,j}
    b_{ij}x_ix_j = \sum\limits_{i,j} b_{ij}x_i^2 +
    ...
    $(x = (x_1,...,x_n))$\\
    Если $\text{rk}f  \Longrightarrow k = r leq n$
    $B = \begin{pmatrix}
        \alpha_1
    \end{pmatrix}
    $ 
    ...
    $$k(x) = \sum\limits_{i=1}^{D}x_i^2 - \sum\limits_
    ...
    \begin{center}
        \textbf{Алгоритм Лагранжа (мметод выделения квадратов.)} 
    \end{center}

    Пусть $k(x_1,...,x_n) = b_{11}x_1^2 +...+ b_{nn}x_n^2 + 
    2b_{12}x_1x_2 +...+ 2b_{1n}x_1x_n +...$\\
    ...
    b_1(x_1 + \sum\limits_{i=2}^{n} \frac{b_{1i}x_i}{b_{11}})
    ^2 + k_1(x_2,...,x_n)$$ 
    ...
    $$\begin{cases}
        ...
        2(x_i'^2 - x_j'^2)\\
        x_j = x_i' + x_j'
    \end{cases} \Longrightarrow \text{ перейти к основному случаю}$$
