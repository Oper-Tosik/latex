...
\begin{document}
    ...
    \begin{proof}
        ...
        \end{pmatrix}\stackrel{\text{Э.П.строк}}{\sim}
    ...
    (a_{ij}) = A_{\varphi,e,f}.$\\
    Обозначение: $y = \varphi(v),\ v = eX_e^\uparrow \Longrightarrow
    ...
    $\\\begin{pmatrix}A\\\hline B\end{pmatrix} \stackrel{\text{Э.П.столбцов}}
    {\sim}\begin{pmatrix}E\\\hline X\end{pmatrix}
    \Longrightarrow X = A_{\varphi,e,f}$\\
    $\\(A^T|B^T) \stackrel{\text{Э.П.строк}}{\sim} (E|X^T)$
    ...
    В качестве $X$ по очереди
    взять столбцы $E_n$, в качестве $Y$ -- столбцы $E_m$.
    
    Если $ \varphi v \longmapsto V$ -- линейный оператор,
    $e, e'$ -- базисы, $C = C_{e \to e'}$, то 
    $a_{\varphi, e'} = C^{-1}A{\varphi,e}C$ (*)
    Термин. Две матрицы называются подобными, если $\exists
    C:\ A' = C_{-1}AC\ (|C| \neq 0)$
    ...
    \begin{subtheorem}
        \begin{enumerate}
            \item $L(V,V')$ является векторным пространством над $F$  
            \item Если $dimV = n,\ dimV' = m$, то $L(V,V') \cong
            M_{m\times n}(F)$  
        \end{enumerate}
    \end{subtheorem}    
    \begin{proof}
        ...
        $Y$ -- столбец координат вектора $\psi(v)$, то
        $Y = A_\psi X, (\varphi + \psi)(v) = \varphi(v) + \psi(v)
        \Longrightarrow \varphi(v) + \psi(v)$ имеет столбец
        координат $Y + Y = A_\varphi X + A_\psi X = 
        (A_\varphi + A_\psi)X = A_{\varphi + \psi}X$    

    \end{proof}

    Умножение (композиция) линейных отображений (операторов).
    ...
    Обозначим L(V) -- множество всех линейных операторов на $V$.
    
    \begin{theorem}
        С операциями $+,\ \lambda\ \cdot,$ и $\cdot$\ $L(V)$ является
    ...
    Геометрическое доказательство неравенства $rk(AB) \leq
    \begin{cases}
        rkA\\rkB
    ...
    Многочлен от линейного оператора:
    ...
    Многочлен $p(t)$ -- аннулирующий многочлен оператора $\varphi$,
    если $p(\varphi) = 0$ (нулевой оператор).
    \begin{theorem}
        Для любого оператора $\varphi: V_n \longmapsto V_n$
        существует аннулирующий многочлен степени $\leq n-1$. 
    \end{theorem}
    \begin{proof}
        Т.к. $dimL(V) = n^2$, то операторы, $\varphi^0 = \varepsilon,
        \varphi, \varphi^2,..., \varphi^{n^2}$ -- ЛЗ. $\Longrightarrow
        \exists a_0, a_1,..., a_{n^2} \in F\ 
        a_0\varepsilon + a_1\varphi +...+a_{n^2}\varphi^{n^2}
    ...
    \begin{definition}
        Подпространство $U \subseteq V$ называется \underline{
    ...
    \begin{definition}
        \underline{Ограничение (сужение) оператора} $\varphi$
        на инвариантное подмножество $U$ -- это оператор. 
    \end{definition}

    Обозначение: $\varphi|_v: U \longmapsto U$, а именно,
    $\forall u \in U$  
    ...
    \begin{enumerate}
        ...
        $\varphi(u_1 + u_2) = u_1$ на $U_1$ вдоль $U_2$.
    ...
    \begin{theorem}
        Если $U_1,..., U_k$ -- $\varphi$-инвариантные.  
    \end{theorem}

    \begin{lemma}
        (Вид матрицы $A-\varphi$ при наличии инвариантов 
        пространства)
        \begin{enumerate}
            ...
            \begin{pmatrix}
                \underset{n\timesm}{B} & \underset{m\times n-m}{D} \\
                \big 0 & C 
            \end{pmatrix}$\\ $dimU_1 = m$ 
