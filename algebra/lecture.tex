...
\begin{document}
    ...
    \end{proof}
    \begin{definition}
        Вектор $x \in V$ называется собственным вектором
        оператора $\varphi$, если $x \neq v$ и $\exists
        \lambda \in F:\ \varphi(x) = \lambda x\ (1)$. 
        Это $\lambda$ называется соббственным значением
        оператора $\varphi$ ($x$ -- собственный вектор
        с соббственным значением $\lambda$).
    \end{definition}
    Для данного собственного значения $\lambda \in F$ 
    обозначается $V_\lambda = \{x \in V|\ \varphi(x) = 
    \lambda x\}\ (2)\ $ (Множество соббственных векторов
    с добавленным $0$).

    $V_\lambda$ -- подпространство в $V$.  
    \begin{definition}
        $V_\lambda$ называется соббственным подпространством
    ...
    \end{proof}
    Замечание: если $\varphi(x) = \lambda x\ (x \neq 0),\ 
    ...
    \lambda f(x)e^{-\lambda x} = 0$ на $\mathbb{R} \Longrightarrow 
    ...
    \begin{proof}
        ...
        x_m = 0\ (I).$ Подествуем на равенство $I$ оператором
        $\varphi:\ \alpha_1\varphi(x_1) +...+ \alpha_m \varphi(x_m) = 0
        \Longleftrightarrow \alpha_1 \lambda_1x_1 +...+
        \alpha_1 \lambda_1x_1 + \alpha_{m-1}\lambda_{m-1}x_{m-1} +
    ...
    \begin{consequense}
        ...
        $\varphi$ образуют базис в $V$.  
    \end{consequense}
    Обратим внимание,  чир в этом базисе $e_1,..,e_n$ матрица
    ...
    (\text{слагаемые сетепени } \leq 2 \text{ по }
    \lambda) +... =\\ (-\lambda)^n + \sum\limits_{i=1}^{n} a_{ii}
    (-\lambda)^{n-1} +...+ |A|$

    $\\$ \underline{Термин:} $det(A_\varphi - \lambda E)$ -- характеритический
    ...
    \end{proof}
    \begin{definition}
        $\chi_\varphi(\lambda) = |A_\varphi -  E|$ -- характеритический
    ...
    \begin{definition}
        ...
        диагональна.
    \end{definition}
    Термин: для характеритического корня $\lambda \in F$:

    геометрическая кратность $\lambda, \textit{\text{гкт}}(\lambda) = 
    dimV_\lambda$,

    алгебраическая кратность $\lambda$ -- это его кратность
    ...
    \begin{proof}
        ...
        $A_{\varphi,e} =
            \begin{pmatrix}
                ...
                \null & \null & \null\\
            \end{pmatrix} \Longrightarrow |A_\varphi - \lambda E| =$
        \end{flushleft}
        \begin{flushright}
            $=\begin{vmatrix}
                \lambda - t & \null & 0 & \vline & \null & \null & \null\\
    ...
    \end{proof}
    Замечание: $dimV_{\lambda_j} = n - rk(A_\varphi - 
    \lambda_j E)$.
